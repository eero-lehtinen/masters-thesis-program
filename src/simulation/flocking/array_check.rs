...
pub fn keep_distance_to_others(world: &mut World) {
    ...
    spatial
        ...
        .for_each(|(cell, items)| {
            let neighbors = spatial.get(cell);
...
impl SpatialStructure {
    ...
    pub fn get(&self, cell: usize) -> [Option<&Vec<(Entity, Vec2)>>; 9] {
        ...
        [
            self.grid.get(up_pos - 1),
            self.grid.get(up_pos),
            self.grid.get(up_pos + 1),
            self.grid.get(cell - 1),
            self.grid.get(cell),
            self.grid.get(cell + 1),
            self.grid.get(down_pos - 1),
            self.grid.get(down_pos),
            self.grid.get(down_pos + 1),
        ]
    }
